...
function Search() {
  ...
  const SearchItem = ({ item }) => {
    const creators = () => {
      ...
      const creatorList = item.attributes.creators.reduce(
    ...
    const description = () => {
      ...
      const descriptionHtml = truncate(
        item.attributes.descriptions[0].description,
        { length: 750, separator: "… " }
      );

      return <p>{ReactHtmlParser(descriptionHtml)}</p>;
    };
      ...
      <div key={item.id} className="panel panel-default post-list">
        <div className="panel-body post-content">
          <header className="post-header">
            <h3 className="work">
              <a href={new URL(item.attributes.url).pathname}>
        ...
        <div className="panel-footer" id={item.id}>
          <span className="meta">
            <time>
              {new Date(item.attributes.dates[0].date).toLocaleDateString(
                "en-US",
                {
                  year: "numeric",
                  month: "long",
          ...
          <a
            href={item.attributes.url + "#disqus_thread"}
            data-disqus-identifier={item.attributes.url}
  ...
  React.useEffect(() => {
    let url =
      "https://api.datacite.org/dois?client-id=datacite.blog&sort=-created&state=findable";
    const params = new URLSearchParams(window.location.search);
    if (params.has("query")) url += "&query=" + params.get("query");
    if (params.has("tag")) url += "&subject=" + params.get("tag");
    if (params.has("page")) url += "&page[number]=" + params.get("page");
