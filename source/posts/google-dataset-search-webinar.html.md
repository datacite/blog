...
#### Do you have an exemplary case of schema.org markup for a data source that has covered many of the things to look out for when indexing sources?
...
#### There are many related entities (from schema.org, or hopefully to be added) that also would benefit by adding to search. Are there plans to create search for other scientific resource, like Linux containers (for which sometimes the line between Dataset and Container can be a bit gray!).
...
#### How do you determine metadata quality?
...
#### Can you give us more information on how indexing works?
...
#### Is there a size limit to the web pages that go into the index? (e.g. the structured data testing tool gives up > 2.5Mb).

This is a more general Search question. Maybe a product forum is a good place to ask.

#### Are you handling "spam", fake pages just implementing schema.org with dataset metadata as a fake to put spam into the search?
...
#### Data cleaning would result in a new version of the data file itself, so how do you approach file versioning?  There must be a cache, so how do you handle updates and synchronization? (When the provider updates the original file)
...
#### How do you currently handle "similar" datasets? We often have hundreds of datasets with very similar metadata, just slight differences in metadata (different location/platform). When searching in Google Dataset Search it mostly shows one random result, but all the similar ones are hidden.
...
#### If we have only metadata records in our repository (but not the actual datasets), would those records be indexed ? And would it have an impact on the search rankings?
...
#### Are there plans to allow search on summaries of the data (e.g., limit search to datasets with a minimum number of rows/sample size, data with a high mean age)? How can Dataset Search learn about means and variables labels?
...
#### All "our" datasets found in Google Dataset Search have the DataCite logo, link to search.datacite.org, “provided by Datacite”, etc.? Will adding structured data markup to our own dataset landing pages change that?
...
#### As a data provider can we have a repository level profile? Not just the dataset creator/author.

That sounds like a good idea. We will consider that as we consider author/creator profiles.

#### Are datasets published as sparql endpoints marked up with the DCAT ontology be included in the dataset search?

Yes.

#### How does the dataset have to be linked in the metadata to make indexing more likely in the future?
...
#### What is allowed in the markdown of the description? Will you render figures (e.g., distributions of variables)? Are there plans to use logo provided in the metadata instead of the logo from the Google Knowledge Graph? The Google Knowledge Graph does for some reason not provide our logo.
...
#### Can Google Dataset Search do a dataset preview of its spreadsheet data since Google Search’s can display spreadsheet data in search results?

Yes, that's among the features that we want to implement.

#### As a demo of how things could work, could you point to examples of a process where someone has discovered data w/ Dataset Search, then manipulated in Data Studio, then shared visualizations, for example?

We don't have such an example, but if someone does, and can post it on a blog somewhere, we'd love to have our users see it!

#### Are there any plans to show more metadata for a dataset? For example, we provide a citation for the dataset which is not shown. Since the paper must be cited when the data is used, it would be very nice to have it shown in the Dataset Search as well. Everything but the abstract seems hidden.
...
#### Do you show variables?

A few things we need to show are not shown yet.

### Data Citation

#### How are data citations counted in Google Scholar?
...
#### Does this mean you are going into a direction of "Google Scholar for Scientific Data"
...
#### Are there are plans to include data citations and their corresponding citation counts into Google Scholar Profiles?
